...
export async function getAllTasksWithUserName(id: string): Promise<AllTasksWithUsers | null> {
    try{
        const result: QueryResult<TasksWithUsers> = await sql<TasksWithUsers>`SELECT T.*, U.EMAIL as createdByEmail, U.FULLNAME as createdByName, U.googlepic as createdByPic FROM TASKS T JOIN DBUSERS U ON T.CREATEDBY = U.ID WHERE T.assignee = ${id} order by T.duedate asc;`;
