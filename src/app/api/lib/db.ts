...
export async function getUser(email: string): Promise<dbUser | null> {
  try {
    const result =
      await sql<dbUser>`SELECT * FROM dbusers WHERE email = ${email}`;
...
export async function getAllTasks(id: string): Promise<AllTask | null> {
  try {
    const result: QueryResult<Task> =
      await sql<Task>`SELECT * FROM tasks WHERE createdby = ${id}`;
...
export async function getTaskByTaskID(
  id: string
): Promise<AllTasksWithUsers | null> {
  try {
    const result: QueryResult<TasksWithUsers> = await sql<TasksWithUsers>`
            SELECT T.*, F.FORMDATA as fromForm, U.EMAIL as createdByEmail, U.FULLNAME as createdByName, U.googlepic as createdByPic 
            FROM TASKS T 
            JOIN DBUSERS U ON T.CREATEDBY = U.ID JOIN FORMBYUSER F ON F.ID = T.FORMID 
            WHERE T.id = ${id} 
...
export async function getAllTasksWithUserNameTaskCard(
  id: string
): Promise<AllTasksWithUsers | null> {
  try {
    const result: QueryResult<TasksWithUsers> = await sql<TasksWithUsers>`
...
export async function getAllTasksCreatedWithUserNameTaskCard(
  id: string
): Promise<AllTasksWithUsers | null> {
  try {
    const result: QueryResult<TasksWithUsers> = await sql<TasksWithUsers>`
...
export async function getFormByUserID(
  id: string
): Promise<AllFormByUser | null> {
  try {
    const result: QueryResult<FormByUserWithUser> =
      await sql<FormByUserWithUser>`
...
export async function getFormByUserIDSelect(
  id: string
): Promise<AllFormByUser | null> {
  try {
    const result: QueryResult<FormByUserWithUser> =
...
export async function getFormByID(
  id: string
): Promise<FormByUserWithUser | null> {
  try {
    const result: QueryResult<FormByUserWithUser> =
      await sql<FormByUserWithUser>`
            SELECT F.*, U.EMAIL as createdByEmail, U.FULLNAME as createdByName, U.googlepic as createdByPic 
